	...
	if (!options.storage) {
		throw new Error(
			"Invalid Configuration\n\n" +
				...
				"." +
				path +
				") require storage to be provided."
		);
	}
...
file.prototype.upload = function(item, file, callback) {
	...
	this.storage.uploadFile(file, function(err, result) {
		...
		debug(
			"[%s.%s] Uploaded file for item %s with result:",
			field.list.key,
			field.path,
			item.id,
			result
...
file.prototype.updateItem = function(item, data, files, callback) {
	...
	if (typeof value === "string" && value.substr(0, 7) === "upload:") {
		...
		uploadedFile =
			this.getValueFromData(files) || this.getValueFromData(files, "_upload");
	}
