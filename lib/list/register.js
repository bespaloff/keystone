...
function register () {
	...
	if (this.get('inherits')) {
		function removeNestedSchema (model) {
			for (var path in model.schema.paths) {
			 
			 if (model.schema.paths[path].$isMongooseArray){
			  for (var _path  in model.schema.paths[path].schema.paths){
			   delete model.schema.paths[path].schema.paths[_path].options._nestedSchema;
			   delete model.schema.paths[path].schema.tree[_path]._nestedSchema;
			  }
			 }
			}
			return model;
		   ...
		   var _tModel = this.get('inherits').model;
		   _tModel = removeNestedSchema(_tModel);
		   var _Model = removeNestedSchema(this);
		   this.model = _tModel.discriminator(this.key, _Model.schema.clone());
	} else {
